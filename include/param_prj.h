 ...
 * If it is a spot value, the decimal is translated to the name, i.e. 0 becomes
 * "Choice" If the enum values are powers of two, they will be displayed as
 * flags, example "0=None, 1=Flag1, 2=Flag2, 4=Flag3, 8=Flag4" and the value
 * is 5. It means that Flag1 and Flag3 are active -> Display "Flag1 | Flag3"
 *
 * Every parameter/value has a unique ID that must never change. This is used
 * when loading parameters from flash, so even across firmware versions saved
 * parameters in flash can always be mapped back to our list here. If a new
 * value is added, it will receive its default value because it will not be
 * found in flash. The unique ID is also used in the CAN module, to be able to
 * recover the CAN map no matter which firmware version saved it to flash. Make
 * sure to keep track of your ids and avoid duplicates. Also don't re-assign IDs
 * from deleted parameters because you will end up loading some random value
...
enum { IDC_OFF, IDC_SINGLE, IDC_ISACAN };

enum _modes { MOD_OFF = 0, MOD_RUN, MOD_LAST };

enum _types { BMS_M3 = 0, BMS_TESLAS = 1, BMS_MAX = 2 };
