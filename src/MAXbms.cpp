...
const int8_t tempMap[49] = {
    79, 75, 71, 68, 65, 62, 59, 56, 54, 52, 49, 47, 45,  43,  41, 39, 38,
    36, 35, 33, 31, 30, 29, 27, 26, 24, 23, 21, 20, 19,  17,  16, 14, 13,
    11, 10, 8,  7,  5,  3,  1,  -1, -2, -5, -7, -9, -12, -15, -18};
...
void MAXbms::SpiTest() {
  ...
  spi_xfer(SPI1,
           0x88); // Set the RX_Error_INT_Enable and RX_Overflow_INT_Enable bits
  DigIo::BatCS.Set();
  ...
  check = 0;
  while (check != 0x21) // If RX_Status = 21h, continue. Otherwise, repeat
                        // transaction until true or timeout
  {
    DigIo::BatCS.Clear();
...
void MAXbms::daisyChainInit() {
  ...
  uint32_t watchdogTimer = 0;
  uint16_t check2 = 0;
  while (check2 != 0x21) // If RX_Status = 21h, continue. Otherwise, repeat
                         // transaction until true or timeout
  {
    DigIo::BatCS.Clear();
    spi_xfer(SPI1, 0x01);          // Read RX_Status register
  ...
  if (!((data[0] == 0x03) && (data[1] == 0x57) && (data[2] == 0x00) &&
        (data[3] == 0x00))) {
    errorByte |= initError;
  ...
  // 14, Read the HELLOALL message that propagated through the daisy-chain and
  // was returned back to the ASCI
  DigIo::BatCS.Clear();
  ...
  if (!((data[0] == 0x57) && (data[1] == 0x00) &&
        (data[2] == numberOfSlaves))) {
    errorByte |= initError;
    // spi_xfer(SPI1,0x69);  //Error
  }
  DigIo::BatCS.Set();
...
void MAXbms::clearBuffers() {
  ...
  uint8_t check3 = 0x01;
  uint8_t loop3 = 1;
  while (loop3 == 1) // If RX_Status[0] (RX_Empty_Status) is false, continue. If
                     // true, then repeat transaction until false.
  {
    DigIo::BatCS.Clear();
    spi_xfer(SPI1, 0x01);          // Read RX_Status register
...
void MAXbms::transmitQueue() {
  ...
  DigIo::BatCS.Clear();
  spi_xfer(SPI1,
           0xB0); // WR_NXT_LD_Q SPI command byte (write the next load queue)
  DigIo::BatCS.Set();
  //::SetInt(Param::LoopState, 12);

  // Check if a message has been received into the receive buffer
  while (loop4 == 1) // If RX_Status[1] is true, continue. If false, then repeat
                     // transaction until true
  {
 ...
 ************************************/
void MAXbms::readAllSlaves(uint8_t dataRegister,
                           bool setupDone) // Read all slaves
{
  ...
  // Verify that the device register data is received correctly during the
  // READALL sequence
  if (!((readRegisterData[0] == command) &&
        (readRegisterData[1] == dataRegister))) {
  ...
  if (setupDone) {
    ...
    if (!(((readRegisterData[(2 * numberOfSlaves + 2)] == 0x00) ||
           (readRegisterData[(2 * numberOfSlaves + 2)] == 0x40)) &&
          (readRegisterData[(2 * numberOfSlaves + 3)] == checkPEC) &&
          (readRegisterData[(2 * numberOfSlaves + 4)] == numberOfSlaves))) {
      errorByte |= mismatchAfter;
      if (readRegisterData[(2 * numberOfSlaves + 2)] != 0x00) {
        readAllSlaves(0x02,
                      false); // Read STATUS of all slaves with checks ignored
  ...
  } else // No errors, clear counters and store data received
  {
    ...
    if ((dataRegister >= 0x20) &&
        (dataRegister <= 0x2B)) // Cell voltage measurements
    {
      storeCellVoltage(dataRegister, readRegisterData);
    }

    if ((dataRegister == 0x2D) ||
        (dataRegister ==
         0x2E)) // Aux voltage measurements (external temperature sensors)
    {
    ...
    if (dataRegister ==
        0x13) // Read SCANCTRL to check if data is ready to be read
    {
 ...
 *****************************************/
void MAXbms::readAddressedSlave(uint8_t dataRegister, uint8_t address,
                                bool setupDone) // Read addressed slave
{
  ...
  if (setupDone) {
    ...
    // Check check-byte, PEC and alive-counter
    if (!((readRegisterData[4] == 0x00) && (readRegisterData[5] == checkPEC) &&
          (readRegisterData[6] == 0x01))) {
      errorByte |= mismatchAfter;
      if (readRegisterData[4] != 0x00) {
        readAddressedSlave(
            0x02, address,
            false); // Read STATUS of addressed slave with checks ignored
        writeAddressedSlave(0x02, 0x0000, address,
                            true); // Clear STATUS register
                                   // Serial.println("STATUS cleared");
      }
    }
  }
 ...
 ***********************************/
void MAXbms::writeAllSlaves(uint8_t dataRegister, uint16_t data,
                            bool setupDone) {
  ...
  // Verify that the device register data is what was written during the
  // WRITEALL sequence
  if (!((readRegisterData[0] == command) &&
        (readRegisterData[1] == dataRegister) &&
        (readRegisterData[2] == lowByte(data)) &&
        (readRegisterData[3] == highByte(data)) &&
        (readRegisterData[4] == PEC))) {
    errorByte |= mismatchBefore;
  }
  if (setupDone) {
    // Check alive-counter
 ...
 ****************************************/
void MAXbms::writeAddressedSlave(uint8_t dataRegister, uint16_t data,
                                 uint8_t address,
                                 bool setupDone) // Write addressed slave
{
  ...
  if (errorByte) // Error
  {
    ...
    // delay(1);
    writeAddressedSlave(dataRegister, data, address,
                        setupDone); // Resend WRITEDEVICE
  } else                            // No errors, clear counters
  {
    resendCounter = 0; // Reset counter
                       // failCounter = 0; // Reset counter
...
void MAXbms::readData() {
  ...
  for (int i = 0; i < numberOfCellsPerSlave; i++) {
    readAllSlaves((0x20 + i),
                  true); // Read CELL 1-numberOfCellsPerSlave of all slaves
  }
  // Need to drive GPIO to force logic gates to connect temp sensors GPIO 0x11
  // set to 0x7001 - NTC1-AIN1 + NTC2-AIN2
 ...
 *********************************************************/
void MAXbms::storeCellVoltage(uint8_t dataRegister,
                              uint8_t readRegisterData[]) {
  for (int i = 0; i < numberOfSlaves; i++) {
    uint16_t measVoltage =
        ((readRegisterData[(2 * numberOfSlaves + 1) - i * 2] << 8) +
         readRegisterData[(2 * numberOfSlaves) - i * 2]);
    measVoltage = (measVoltage >> 2) * (uint32_t)5000 / 0x3FFF;
    cellVoltage[i * numberOfCellsPerSlave + (dataRegister - 0x20)] =
        measVoltage;
    Param::SetFloat((Param::PARAM_NUM)(Param::u1 + (i * numberOfCellsPerSlave +
                                                    (dataRegister - 0x20))),
                    measVoltage);
  }
}
 ...
 ***************************************************************************/
void MAXbms::storeCellTemperature(uint8_t dataRegister,
                                  uint8_t readRegisterData[]) {
  for (int i = 0; i < numberOfSlaves; i++) {
    /* Old KIA Code
    //uint16_t beta = 3800;
    uint16_t dataADC = ((readRegisterData[(2 * numberOfSlaves + 1) - i * 2] <<
    8) + readRegisterData[(2 * numberOfSlaves) - i * 2]); dataADC = (dataADC >>
    10);  // >> 4 for 12bit value & >> 6 for correct array index size
    //int8_t temperature = beta / (log((float)dataADC / (4095 - dataADC)) + beta
    / 298.15) - 273; if (dataADC < 7)
    {
        dataADC = 7;
    }
    if (dataADC > 55)
    ...
    */
    ...
    uint16_t measTemperature =
        uint16_t(uint16_t(readRegisterData[3 + (i * 2)] << 4) +
                 uint16_t(readRegisterData[2 + (i * 2)] >> 4));
    ...
    if (TempToggle) {
      if (0x2E == dataRegister) // Toggled is NTC3
      {
        ...
        // Param::SetFloat((Param::PARAM_NUM)(Param::Cellt1_0 +
        // i*2),cellBlockTemp[i * 3 + 2]); // For now use Temp 3 as 2
      }
    } else {
      if (0x2D == dataRegister) // Not toggled so NTC1
      {
        cellBlockTemp[i * 3] = float(measTemperature - 1680) / 32;
        Param::SetFloat((Param::PARAM_NUM)(Param::Cellt0_0 + i * 2),
                        cellBlockTemp[i * 3]);
        // Param::SetFloat(Param::Cellt2_0,measTemperature);
        // Param::SetFloat((Param::PARAM_NUM)(Param::Cellt0_0 +
        // i*2),cellBlockTemp[i * 3]); //Not reading correctly right now
      } else if (0x2E == dataRegister) // Not toggled so NTC2
      {
        ...
        // Param::SetFloat((Param::PARAM_NUM)(Param::Cellt0_1 +
        // i*2),cellBlockTemp[i * 3 + 1]); // For now use Temp 2 as1
      }
    }

    if (0x2E == dataRegister) // NTC2 or NTC3
    {
      if (cellBlockTemp[i * 3 + 1] >
          cellBlockTemp[i * 3 + 2]) // if NTC 2 is higher then NTC 3
      {
        Param::SetFloat((Param::PARAM_NUM)(Param::Cellt0_1 + i * 2),
                        cellBlockTemp[i * 3 + 1]);
      } else {
        Param::SetFloat((Param::PARAM_NUM)(Param::Cellt0_1 + i * 2),
                        cellBlockTemp[i * 3 + 2]);
...
void MAXbms::setupSlaves() {
  ...
  // Enable measurement
  writeAllSlaves(0x12, 0x3FFF,
                 true); // Set MEASUREEN, Enable cell voltages 1-12 and AUX1-2
  writeAllSlaves(0x1E, 0x000C,
                 true); // Set TOPCELL, Top cell for measurement is 12
  writeAllSlaves(0x51, 0x0006,
                 true); // Set DIAGCFG, Enable Die temperature measurement
  // writeAllSlaves(0x19, 0x003F, true); // Set ACQCFG, 64x6us settling time
  // before AUX1 measurement writeAllSlaves(0x18, 0x1500, true); // Set WATCHDOG,
  // Set watchdg for cell balancing to 5s writeAllSlaves(0x13, 0x0001, true); //
  // Set SCANCTRL, SCAN Switch on THRM voltage
  writeAllSlaves(
      0x19, 0x033F,
      true); // Set ACQCFG, Maunal Thrmmode AUX voltage, set measure to max.
}

uint8_t MAXbms::lowByte(int data) { return (data & 0xFF); }

uint8_t MAXbms::highByte(int data) { return ((data >> 8) & 0xFF); }
...
void MAXbms::UpdateStats() {
  ...
  {
    for (int g = 0; g < numberOfCellsPerSlaveMax;
         g++) // go through all voltages
    {
      if (cellVoltage[(h * numberOfCellsPerSlaveMax) + g] >
          MaxVoltTemp) // check max volt
      {
        MaxVoltTemp = cellVoltage[(h * numberOfCellsPerSlaveMax) + g];
        CellMax = CellCnt + 1;
      }
      if (cellVoltage[(h * numberOfCellsPerSlaveMax) + g] >
          1000) // 1000mV threshold check cell present
      {
        CellCnt++;
        Vtotal = Vtotal + cellVoltage[(h * numberOfCellsPerSlaveMax) + g];

        if (cellVoltage[(h * numberOfCellsPerSlaveMax) + g] <
            MinVoltTemp) // check min volt
        {
  ...
  Param::SetInt(Param::uavg, (Param::GetFloat(Param::udc) /
                              Param::GetInt(Param::CellsPresent) * 1000));
